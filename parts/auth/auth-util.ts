...
import { type FastifyAuthConfig } from "@mridang/fastify-auth";
 ...
 */
async function refreshAccessToken(ppc: PPC, token: JWT): Promise<JWT> {
  ...
  try {
    const oidcConfig = await oidc.discovery(
      new URL(ppc.config.zitadel.domain),
      ppc.config.zitadle.client_id,
      ppc.config.zitadel.client_secret,
    );
...
export async function buildLogoutUrl(
  ...
  const urlObj = oidc.buildEndSessionUrl(oidcConfig, {
    ...
    post_logout_redirect_uri: ppc.config.zitadel.post_logout_url,
    state,
  });
 ...
 */
export function authConfig(ppc: PPC): FastifyAuthConfig {
  return {
    useSecureCookies: ppc.config.is_production,
    basePath: "/auth",
    trustHost: true,
    providers: [
      Zitadel({
        issuer: ppc.config.zitadel.domain,
        clientId: ppc.config.zitadel.client_id,
        clientSecret: ppc.config.zitadel.client_secret,
        authorization: {
          params: {
            scope: ppc.config.zitadel.scopes,
          },
        },
      }),
    ],
    ...
    session: {
      strategy: "jwt",
      maxAge: Number(ppc.config.auth.session_duration),
    },

    secret: ppc.config.auth.session_secret,
    ...
    callbacks: {
      ...
      async jwt(params: {
        ...
        return refreshAccessToken(ppc, token);
      },
       ...
       */
      async session(params: { session: any; token: JWT }): Promise<any> {
